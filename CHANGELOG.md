...
- **Feature & Engine: Playing Cards From Non-Hand Zones & `PLAY_CARD_FROM_ZONE` Primitive ([#25](https://github.com/SteveRodrigue/MCD/issues/25), [ADR-0047](docs/decisions/0047-playing-cards-from-non-hand-zones.md), RR v1.8 p. 19, `actions.ts`, `legality-checker.ts`, `action-dispatcher.ts`, `effects/index.ts`, `schema.ts`, `schema.json`, `core.json`, `tests/engine/make-the-call.test.ts`):**
  ...
  - **Automated Contract Tests:** Authored 5 contract tests in `tests/engine/make-the-call.test.ts` covering atomic targeted dispatch, cross-player discard selection under caller control, decision prompt queuing and resolution, out-of-hand play via `sourceZone: PLAYER_DISCARD`, and ally limit constraints. Full test suite (84 test files, 575 tests) passing cleanly.

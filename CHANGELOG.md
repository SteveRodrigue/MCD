...
- **Fix (UI & Engine): Dynamic Board Stats Synchronization & The Triskelion Ally Limit Display ([RR v1.8 p. 3, 11](docs/algorithmic_rules_reference.md), [ADR-0018](docs/decisions/0018-declarative-state-modifiers-and-dynamic-board-limits.md), [#103](https://github.com/SteveRodrigue/MCD/issues/103)):**
  - **Root Cause & Diagnosis:** In `HeroZone.tsx`, the Allies in Play counter was hardcoded as `Allies in Play ({player.allies.length} / 3)`. Even though the engine's `legality-checker.ts` properly permitted playing a 4th ally when *The Triskelion* (`01073`) was in play, the UI header failed to reflect the dynamic capacity increase. Similarly, `GameBoard.tsx` passed printed `(player.activeFormCard.handSize ?? 6)` to `PlayerHandTray` rather than evaluating `getEffectiveHandSize(player, gameState)` (obscuring *Tech* upgrade hand bonuses for Iron Man `01029a`), and `VillainZone.tsx` rendered raw unbuffed card stats (`villain.card.attack`/`scheme`) rather than `getEffectiveVillainStats` (obscuring attachment buffs like *Enhanced Ivory Horn* and *Charge*).

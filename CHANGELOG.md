...
- **Fix (Engine & UI): Ability resource cost payment, dynamic counter scaling, and zero-counter blast guard ([RR v1.8 p. 3, 20, 24](docs/algorithmic_rules_reference.md), [#96](https://github.com/SteveRodrigue/MCD/issues/96)):**
  ...
  - **Universal Reusable Primitives:**
    ...
    - **Dynamic `PER_RESOURCE_SPENT` Scaling:** Supported `scaling: "PER_RESOURCE_SPENT"` in `action-dispatcher.ts` so variable abilities (*"Spend X [energy] resources -> put X energy counters here"*) dynamically scale based on paid resources without an artificial placement cap, in strict adherence to RR v1.8 p. 20 overpaying rules.
  ...
  - **Supplemental Data Audit:** Updated `01018` (*Energy Channel*) in `src/data/supplemental/pack/core.json` to use `scaling: "PER_RESOURCE_SPENT"` and updated audit metadata.
  - **Contract & Regression Tests:** Added comprehensive test suite in `tests/engine/action-cost-engine.test.ts` verifying rejection when lacking matching resources, spending 1 energy card, wild substitution, multi-resource payment (*Energy* `01088` adding 2 counters), overpayment allowing 6+ counters, 0-counter blast rejection, and blast damage resolution (including clamping 6 counters to the printed 10 damage cap).
...
- **Fix (Engine): Encounter card revelation pipeline trigger gating and Treachery timing check ([RR v1.8 p. 22, 25](docs/algorithmic_rules_reference.md), [#97](https://github.com/SteveRodrigue/MCD/issues/97)):**
  - **Root Cause & Diagnosis:** In `src/engine/pipeline/villain-phase.ts`, encounter card revelation (`resolveActiveEncounterCardAfterInterrupt`) blindly executed any card ability with `timing === 'FORCED_RESPONSE'` immediately upon card revelation, ignoring the ability's trigger event. When Sandman (`01102`) spawned into play, his `MINION_ATTACKED` Forced Response (`sandman_attack_discard`) fired immediately upon entering play, milling the top 2 cards of the encounter deck (*Eviction Notice* `01165` and *"I'm Tough!"* `01105`) directly into the discard pile. Furthermore, the Treachery revelation check contained a copy-paste check for `timing === 'FORCED_RESPONSE'` instead of `timing === 'WHEN_REVEALED'`.
  ...
  - **Contract & Regression Tests:** Added regression tests in `tests/engine/villain-phase.test.ts` asserting Sandman entering play does not trigger his post-attack discard (preserving the encounter deck), and verifying *"I'm Tough!"* (`01105`) correctly applies Tough status to Rhino when not tough and triggers Surge when Rhino is already tough.
...
- **Fix (Engine): Universal in-play self-referential trigger instance binding for `ENTERS_PLAY` and `CARD_PLAYED` ([ADR-0050](docs/decisions/0050-universal-in-play-self-referential-trigger-instance-binding.md), [#93](https://github.com/SteveRodrigue/MCD/issues/93)):**
  ...
  - **Ally Action Preservation:** Preserved ally action scoping for `THWART_RESOLVED` and `ATTACK_RESOLVED`, ensuring hero-watching upgrades in the tableau (e.g. *Superhuman Strength* `01028`) continue to trigger correctly on hero basic attacks without regression.
...
- **Engine & Schema: Universal CONSUME_INTERCEPTED_EVENT & Scalar Value Binding ([ADR-0049](docs/decisions/0049-composable-value-transformers-and-event-interception.md), [#90](https://github.com/SteveRodrigue/MCD/issues/90)):**
  ...
  - **Supplemental Retrofits:** Migrated *Great Responsibility* (`01061`), *Emergency* (`01085`), and *Backflip* (`01003`) in `src/data/supplemental/pack/core.json` to declarative `CONSUME_INTERCEPTED_EVENT` + dynamic amount primitives.
...
- **Feature & Engine: Universal Encounter Card Reveal Cancellation & `CANCEL_WHEN_REVEALED` Primitive ([#1](https://github.com/SteveRodrigue/MCD/issues/1), RR v1.8 p. 7, 16, 31-32, `villain-phase.ts`, `trigger-dispatcher.ts`, `prompt-queue.ts`, `effects/index.ts`, `pack/core.json`, `tests/engine/cancel-when-revealed.test.ts`):**
  ...
  - **Card Supplemental Retrofit (`src/data/supplemental/pack/core.json`):** Retrofitted Spider-Man event *Enhanced Spider-Sense* (`01004`) to trigger at `TREACHERY_REVEALED`, timing `HERO_INTERRUPT`, zone `HAND`, with cost `{ discardSelf: true }` and step effect `CANCEL_WHEN_REVEALED`.
...
- **Fix & Engine: Universal Play Requirements Architecture and Form Gating ([#81](https://github.com/SteveRodrigue/MCD/issues/81), RR v1.8 p. 16, `legality-checker.ts`, `schema.ts`, `schema.json`, `abilities.ts`, `pack/core.json`, `AbilityFormBuilder.tsx`, `10_play_requirements.md`, `tests/engine/attachments-player.test.ts`):**
  ...
  - **Engine Legality Checker Integration (`src/engine/pipeline/legality-checker.ts`):** Wired universal `evaluatePlayRequirements` into `canPlayCard` and `evaluateCardPlayability`. Correctly enforces form restrictions for upgrades/attachments with printed form restrictions (fixing *Webbed Up* `01009` being erroneously playable in Alter-Ego form).
  - **Zero Tech Debt Elimination:** Completely purged hardcoded card check for *Wakanda Forever!* (`01043a-d`) from `legality-checker.ts` in favor of declarative `controlFilter: { traits: ['Black Panther'], types: ['upgrade'] }`, and deprecated untyped `playRestriction` properties.
  ...
  - **Automated Contract & Regression Tests:** Added unit tests in `tests/engine/attachments-player.test.ts` validating that `01009` Webbed Up is strictly prohibited in Alter-Ego form and allowed in Hero form, and verified *Wakanda Forever!* sequences in `tests/engine/wakanda-forever-sequence.test.ts`. Full test suite (82 files, 567 tests) passing.
...
- **Feature & Architecture: Universal Declarative Card Filtering Architecture ([ADR-0046](docs/decisions/0046-universal-declarative-card-filtering-architecture.md), RR v1.8 p. 19, 26, 28, `card-filter.ts`, `schema.ts`, `schema.json`, `effects/index.ts`, `stat-calculator.ts`, `game-setup.ts`, `AbilityFormBuilder.tsx`, `effect-parameter-registry.ts`, `04_universal_card_filter.md`, `tests/engine/filters/card-filter.test.ts`):**
  ...
  - **Catalog Supplemental Retrofit:** Retrofitted all consuming Core Set cards (*Black Cat* `01002`, *Iron Man* `01029a`, *T'Challa* `01040b`, *Shuri* `01041`, *Electric Whip* `01173`, *Shadow of the Past* `01190`, *Caught Off Guard* `01188`) to the canonical universal card filter schema.
...
- **Feature & Engine: Unify Fragmented `EXHAUST_*` and `READY_*` Primitives into Universal `EXHAUST` and `READY` with Target Selectors ([#65](https://github.com/SteveRodrigue/MCD/issues/65), RR v1.8 p. 13, 23, `schema.ts`, `schema.json`, `effects/index.ts`, `effect-parameter-registry.ts`, `AbilityFormBuilder.tsx`, `06_effects_status_economy.md`, `03_costs_and_targeting.md`, `tests/engine/universal-exhaust-ready.test.ts`):**
  ...
  - **Card Supplemental Retrofit (Confidence: 50%):** Retrofitted all 6 affected Core Set cards to canonical schemas with audit confidence set to 50% pending extensive gameplay verification: *One-Two Punch* (`01024`), *Arc Reactor* (`01035`), *Get Ready* (`01069`), *Indomitable* (`01082`), *Tenacity* (`01093`), and *Exhaustion* (`01191`).
...
- **Refactor & Engine: Unify Fragmented `DISCARD_*` Primitives into Universal `DISCARD` ([#66](https://github.com/SteveRodrigue/MCD/issues/66), RR v1.8 p. 10, `schema.ts`, `schema.json`, `effects/index.ts`, `effect-parameter-registry.ts`, `05_effects_zones_cards.md`, `pack/core.json`, `pack/core_encounter.json`, `tests/engine/universal-discard-engine.test.ts`):**
  ...
  - **Declarative Supplemental Retrofit (Confidence: 50%):** Retrofitted 12 cards across player and encounter supplemental data (*Black Cat* `01002`, *Legal Practice* `01023`, *Nick Fury* `01084`, *Enhanced Ivory Horn* `01100`, *Sandman* `01103`, *Spider-Man's Orders* `01155`, *Depowered* `01160`, *The Price of Failure* `01166`, *Family Feud* `01170`, *Electrified Webbing* `01174`, *Unstable Matter* `01175`, *Caught Off Guard* `01188`, and *Shadow of the Past* `01195`) with audit confidence set to 50% pending extensive gameplay verification.
...
- **Refactor & Engine: Unify Redundant Counter/Token Cost Primitives into Canonical `spendCounters` ([#74](https://github.com/SteveRodrigue/MCD/issues/74), ADR-0035, `schema.ts`, `schema.json`, `abilities.ts`, `cost-engine.ts`, `03_costs_and_targeting.md`, `tests/data/supplemental-schema.test.ts`):**
  ...
  - **Zero Card Blast Radius:** Confirmed zero usages of obsolete fields across all supplemental pack files; all active player cards (*Web-Shooter* `01008`, *Tac Team* `01056`, *Surveillance Team* `01064`, *Med Team* `01080`) are already using canonical `spendCounters`.
...
- **Feature & Engine: Player Decision Prompts on Optional Interrupts and Responses ([#77](https://github.com/SteveRodrigue/MCD/issues/77), RR v1.8 p. 8, 11, 15, `trigger-dispatcher.ts`, `combat-pipeline.ts`, `action-dispatcher.ts`, `prompt-queue.ts`, `state.ts`, `tests/engine/optional-triggers.test.ts`):**
  ...
  - **Sequential Combat Pipeline Pausing & Continuation:** Extended the 7-step combat state machine to pause upon Step 2 initiation trigger prompts (e.g. *Spider-Sense* `01001a`) and Step 6 damage mitigation interrupt prompts (e.g. *Backflip* `01003`). Added `continueAttackAfterInitiation` and `finishAttackDamageAndPostResolution` to seamlessly resume defense declaration, boost cards, and damage calculation upon player prompt resolution.
  ...
  - **Thwart Response Trigger Dispatching:** Connected `ALLY_THWART` to `dispatchTrigger('THWART_RESOLVED')` with `sourceInstanceId`, allowing allies like *Daredevil* (`01058`) to trigger their optional damage responses upon thwarting.
...
- **Feature & Engine: Support Minion & Engaged Enemy Target Taxonomy (`CHOSEN_MINION`, `CHOSEN_ENGAGED_MINION`, `ALL_MINIONS`, `ENGAGED_ENEMIES`) and Enforce Minion Play Legality ([#76](https://github.com/SteveRodrigue/MCD/issues/76), `schema.ts`, `schema.json`, `legality-checker.ts`, `cost-engine.ts`, `state-validator.ts`, `action-dispatcher.ts`, `03_costs_and_targeting.md`, `tests/engine/attachments-player.test.ts`):**
  ...
  - **Play Legality Enforcement:** Enforced RR v1.8 p. 6, 19, 28 across `canPlayCard` and `evaluateCardPlayability`: cards and actions requiring minions (e.g. *Spider-Tracer* `01007`, *Relentless Assault* `01053`) are illegal to play and excluded from legal actions when no valid minion exists on the table.

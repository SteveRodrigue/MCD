...
- **Status:** Accepted (Updated with Card Analysis Methodology, Scanned-Only Policy & `noSupplementalNeeded` Signal)
- **Date:** 2026-08-26
...
- Drawing cards upon a trigger condition (e.g. _Spider-Sense_, _Avengers Mansion_, _One-Two Punch_).
- Preventing attack damage (e.g. _Backflip_, _Side Step_, _Energy Barrier_).
- Dealing damage to enemies with keywords/tags (e.g. _Swinging Web Kick_, _Haymaker_, _Shield Toss_).
- Healing damage via actions (e.g. _Aunt May_, _First Aid_, _Med Team_).
- Resource generation via counters or tapping (e.g. _Web-Shooter_, _Helicarrier_, _Enhanced Reflexes_).
...
- Subtle timing distinctions (_Interrupt_ vs _Forced Interrupt_ vs _Response_ vs _Forced Response_ vs _Constant_).
- Implicit vs explicit costs (_"Exhaust and spend 1 [mental]"_ vs _"Take 1 damage to draw 1 card"_).
- Replacement effects (_"When damage would be dealt... prevent it instead"_).
- Unique card-specific mini-games (_Repulsor Blast discarding top 5 cards and counting Energy icons, Nick Fury choosing from 3 distinct branches, Wakanda Forever executing all upgrades in player-chosen order_).
...
- **If the card operates 100% on printed stats without rules text or triggers** (e.g. _Energy_, _Genius_, _Strength_, _Energy Absorption_, _Vibranium_, or vanilla schemes/minions), mark `"noSupplementalNeeded": true`.
...
- Extract the timing keyword:
  ...
  - `RESOURCE`: Activated specifically during resource payment windows (e.g. _Scientist_, _Web-Shooter_, _Pepper Potts_).
  - `SPECIAL`: Triggered indirectly by other cards (e.g. _Wakanda Forever!_ executing Black Panther upgrades).
...
- Deconstruct activation costs and constraints:
  ...
  - `removeCounter`: Decrements counters from `uses` (e.g. _Web-Shooter_, _Tac Team_, _Hawkeye_, _Med Team_).

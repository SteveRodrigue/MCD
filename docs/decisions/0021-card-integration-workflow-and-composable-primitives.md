...
3. **Draft Supplemental Schema & Audit Block:** Define explicit `audit` (`createdAt`, `updatedAt`, `reviewedAt`, `rulesVersion`, `confidence`, and proof-of-work `reconstructedText`), `mechanicSteps`, and **mandatory executable `abilities: [...]` array** (`id`, `timing`, `trigger`, `cost`, `effect`, `params`, `tags`). `noSupplementalNeeded: true` is strictly reserved for vanilla cards with 0 rules text.

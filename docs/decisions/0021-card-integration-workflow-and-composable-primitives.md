# ADR-0021: Standard Card Integration Protocol, Composable Primitives & Inbox-Zero Ambiguity Tracking

## Status
Accepted

## Context & Problem Statement
When digitizing card games with extensive card pools like Marvel Champions, three failure modes frequently arise:
...
To guarantee fidelity to **Marvel Champions Rules Reference v1.8** and maintain a clean, extensible codebase, we require a standardized engineering protocol, a strict circuit-breaker, and a 1-file-per-card ambiguity tracking queue.

---

## Decision

We establish the **8-Step Card Integration Protocol**, **Circuit-Breaker Rule**, and **1-File-Per-Card Ambiguity Queue (`docs/ambiguities/`)**:
...
8. **Codify `mechanicSteps`, Document Specs & Prune Ambiguity:** Populate `mechanicSteps` in JSON, document specs in `docs/specs/card-mechanics-breakdown.md`, and **delete the corresponding ambiguity file** in `docs/ambiguities/` upon successful resolution (Inbox Zero).

### 2. Inbox Zero Ambiguity Queue (`docs/ambiguities/`)
...
### 3. Composable Primitives Design Pattern

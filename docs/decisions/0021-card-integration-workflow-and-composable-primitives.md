...
8. **Stamp Audit (HH:MM & reconstructedText), Codify Specs, Sort Keys & Prune Ambiguity:** Stamp ISO timestamps (`YYYY-MM-DDTHH:mm`), record `reconstructedText` proof of work, populate `mechanicSteps` in JSON, sort supplemental JSON keys canonically by ascending card ID (`01001a` -> `01001b` -> `01002`), document specs in `docs/specs/card-mechanics-breakdown.md`, and **delete the corresponding ambiguity file** in `docs/ambiguities/` upon successful resolution (Inbox Zero).

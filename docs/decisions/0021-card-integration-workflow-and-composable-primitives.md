...
2. **Ad-Hoc Interpretation / Guesswork:** Developers informally guessing or assuming card interactions, misinterpreting exact scopes (e.g. conflating _Villain Schemes_ with _Minion Schemes_, or missing identity resource timings).
...
3. **Draft Supplemental Schema & Audit Block:** Define explicit `audit` (`createdAt`, `updatedAt`, `reviewedAt`, `rulesVersion`, `confidence`, and proof-of-work `reconstructedText`), `mechanicSteps`, and **mandatory executable `abilities: [...]` array** with **Fully Qualified Game Zones (FQGZ)** (e.g. `searchZones: ["ENCOUNTER_DECK", "ENCOUNTER_DISCARD"]`, `shuffleDeck: "ENCOUNTER_DECK"`). Unqualified generic strings like `["DECK", "DISCARD"]` are strictly prohibited. `noSupplementalNeeded: true` is strictly reserved for vanilla cards with 0 rules text.
4. **Consult Ground Truth (`references/`):** Check `references/mc_rulesreference_v18_compressed.pdf` and MarvelCDB FAQs (`https://marvelcdb.com/faqs`). Apply **The Golden Rule** (Card text overrules general rules).
5. **Round-Trip Decompiler Feedback Loop & Circuit-Breaker:** Translate drafted `abilities: [...]` array back into human language (ignoring comments/steps) and compare directly against printed card text. If the executable schema cannot recreate a comparable card text with 100% equivalent meaning and intent, confidence CANNOT be rated $\ge 95\%$.
   ...
   - **Active Abilities Isolation Rule:** For any card with an open ambiguity, the **`abilities: [...]` array MUST be omitted / stripped** from supplemental JSON. The entry retains only `comment`, `audit` (with `ambiguityFile`), and `mechanicSteps` to prevent runtime engine execution of unsupported logic.
6. **Code-Level Primitive & Trigger Path Implementation Audit:** Open `src/engine/effects/index.ts` and `src/engine/pipeline/` to verify that the TypeScript code genuinely implements all mechanical clauses (deck/discard search, secondary side-effects like shuffling, player scaling) and confirms that the trigger event is actively emitted at runtime for that entity type.
...
8. **Stamp Audit (HH:MM & reconstructedText), Codify Specs, Sort Keys & Prune Ambiguity:** Stamp ISO timestamps (`YYYY-MM-DDTHH:mm`), record `reconstructedText` proof of work, populate `mechanicSteps` in JSON, sort supplemental JSON keys canonically by ascending card ID (`01001a` -> `01001b` -> `01002`), document specs in `docs/specifications/card_mechanics_breakdown.md`, and **delete the corresponding ambiguity file** in `docs/ambiguities/` upon successful resolution (Inbox Zero).

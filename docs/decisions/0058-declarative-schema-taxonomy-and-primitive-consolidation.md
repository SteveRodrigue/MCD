...
`ENEMY_INITIATES_ATTACK` is a universal event, not a synonym for villain attack. Its context must identify attacker kind (`VILLAIN` or `MINION`), the concrete attacker instance (`sourceInstanceId`), the source card definition ID (`sourceCardCode`, such as `01094`), attacked player, and engagement scope. Ability declarations use a universal `triggerFilter` predicate, such as `{ attackerKind: 'VILLAIN', targetPlayerScope: 'SELF' }` for Spider-Sense, so villain-only, minion-only, enemy-wide, and engaged-player reactions do not depend on which event names the pipeline happens to dispatch. The same mechanism applies to damage, defeat, and form-change context fields; missing context fails closed. This trigger-filter contract is a required follow-up, not yet an implemented Phase 2 capability.

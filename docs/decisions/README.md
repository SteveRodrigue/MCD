...
## 🗺️ Visual ADR Lineage & Evolution Graphs

Each chart below covers one architectural lineage. An arrow means _"was built on / enabled by"_; a
labelled `Superseded by` arrow reflects a `Superseded` status recorded in both the ADR file and the
Decision Log table. **Every edge shown is backed by an explicit ADR-to-ADR reference in the ADR body
or by its recorded supersession status** — no relationships are inferred.

### 1. Foundation & Core Invariants

_The two root decisions — a headless engine and the official rules as sole authority — and the
structural invariants derived from them._

```mermaid
graph TD
    ADR02["ADR-0002: Headless Decoupled Engine"] --> ADR27["ADR-0027: Modular Phase Pipelines"]
    ADR02 --> ADR29["ADR-0029: Generic Zone Primitives"]
    ADR02 --> ADR31["ADR-0031: 5-Phase Combat & Multi-Window Defense"]
    ADR02 --> ADR32["ADR-0032: Frame Stack & Prompt Queue"]
    ADR07["ADR-0007: Rules Authority (RR v1.8)"] --> ADR19["ADR-0019: Zero Raw-Text Parsing"]
    ADR19 --> ADR29
    ADR19 --> ADR33["ADR-0033: 15-Step Scenario Plugin Engine"]
```

### 2. Ability & Schema Lineage

_How card abilities are declared: from flat `effect`/`params` pairs, through nested sequences, to
today's unified `steps: AbilityStep[]` array._
...
_Enemy attacks, defense windows, and the nested interrupt/response machinery that resolves them._

```mermaid
graph TD
    ADR27["ADR-0027: Modular Phase Pipelines"] --> ADR31["ADR-0031: 5-Phase Combat & Multi-Window Defense"]
    ADR19["ADR-0019: Zero Raw-Text Parsing"] --> ADR31
    ADR20["ADR-0020: Interactive Interrupts (Single Prompt)"] -->|Superseded by| ADR32["ADR-0032: Frame Stack & Prompt Queue"]
    ADR30["ADR-0030: Unified AbilityStep[]"] --> ADR32
    ADR31 --> ADR32
    ADR31 --> ADR36["ADR-0036: Advanced Status (Stalwart/Steady) & Minion Modifiers"]
```

### 4. Scenario Setup & Content Expansion

_Replacing ad-hoc scenario wiring with the official 15-step plugin pipeline, and the content zones
layered on top of it._

```mermaid
graph TD
    ADR10["ADR-0010: Scenario Catalog (Ad-hoc)"] -->|Superseded by| ADR33["ADR-0033: 15-Step Scenario Plugin Engine"]
    ADR29["ADR-0029: Generic Zone Primitives"] --> ADR33
    ADR33 --> ADR34["ADR-0034: Player Side Schemes & Victory Display"]
    ADR18["ADR-0018: Declarative State Modifiers"] --> ADR35["ADR-0035: Multi-Form Identities & Counter Engine"]
    ADR30["ADR-0030: Unified AbilityStep[]"] --> ADR35
```

### 5. Presentation & Localization

_The comic-book presentation layer, which composes the art direction, i18n, and action-log decisions._
...
_ADR-0038 through ADR-0046 are Accepted and in force, but their files contain **no ADR-to-ADR
...
```mermaid
graph TD
    subgraph Abilities["Ability & Identity Extensions"]
    ...
    subgraph DevTooling["Developer Tooling & Data Quality"]
        ...
        ADR43["ADR-0043: Supplemental Schema Validation & Live VS Code Integration"]
...
| [ADR-0040](0040-universal-card-conservation-and-atomic-zone-transfer.md)                          | 2026-09-01 | Universal Card Conservation, Atomic Zone Transfer & State Invariant Engine           | **Accepted**                                                                                                 | Guarantee physical card identity uniqueness (exactly one zone per active card instance) with atomic zone removal and fail-fast state invariant assertion.                                                             |
...
| [ADR-0042](0042-local-first-developer-problem-reporting.md)                                       | 2026-09-02 | Local-First Developer Problem Reporting & Deferred GitHub Filing                     | **Accepted**                                                                                                 | Capture Dev Mode "Report a Problem" submissions (GameState attached) as local JSON via a dev-only Vite middleware instead of embedding a GitHub token client-side; a companion skill files them to GitHub later.     |
| [ADR-0043](0043-codebase-grounded-supplemental-schema-validation-and-live-vscode-integration.md)   | 2026-09-03 | Codebase-Grounded Supplemental Schema Validation & Live VS Code Integration           | **Accepted**                                                                                                 | Restrict supplemental abilities to strict codebase-grounded Zod effect enums, export Draft JSON Schema to `schema.json`, configure live VS Code diagnostics, and enforce CI/CD parity tests.                         |
| [ADR-0044](0044-card-text-parsing-and-declarative-mapping-analyzer.md)                         | 2026-09-03 | Card Text Parsing & Declarative Mapping Analyzer Tool                                | **Accepted**                                                                                                 | Implement a deterministic regex pattern engine and CLI tool to normalize and parse printed card text into valid Zod supplemental declarations.                                                                        |
| [ADR-0045](0045-card-supplemental-editor-and-live-reviewer-gui.md)                             | 2026-09-05 | Card Supplemental Editor & Live Data Reviewer GUI                                    | **Accepted**                                                                                                 | Implement a dual-inspector visual reviewer, multi-criteria filtering, local dev REST middleware, live Zod validation, and in-game right-click context menu integration.                                                |
| [ADR-0046](0046-universal-declarative-card-filtering-architecture.md)                         | 2026-09-06 | Universal Declarative Card Filtering Architecture                                    | **Accepted**                                                                                                 | Establish universal composable predicate criterion tree, deprecating legacy ad-hoc filtering and retrofitting supplemental data with zero tech debt. |

...
- [x] **6.6 — `<CostBuilder>` Section (extend `AbilityFormBuilder.tsx`):** Checkboxes for `exhaustSelf`/`discardSelf`, resource-type chip picker for `resources`/`resourceCost`, `spendCounters` sub-form, `discardCard` sub-form, `damageHero`/`damageSelf` numeric inputs.

...
- [x] **Universal SEARCH_AND_SELECT Two-Pile Destination Routing & Specific Card Picking (RR v1.8 p. 19, 26 / [ADR-0030](decisions/0030-unified-ability-step-sequence-architecture.md) / [Issue #10](https://github.com/SteveRodrigue/MCD/issues/10)) ✅ (Completed):**
  ...
  - **Multi-Zone Search & 0/'ALL' Pool ([#115](https://github.com/SteveRodrigue/MCD/issues/115)) ✅ (Completed):** Extended `SEARCH` to support multiple source zones (`SearchZone[]`), unified `0` and `'ALL'` semantics (`lookCount` 0 = entire pile, `takeCount` 0 = all matches without prompt), voluntary player search "Pass" option vs mandatory forced encounter resolution, encounter card reveal routing (`REVEAL`), non-negative validation (`min={0}`), and multi-select chip picker in the Card Editor. Retrofitted Rhino Stage II (`01095`).
...
- [x] **Core Hero Signature Mechanics Completed:**
  ...
  - Captain Marvel (`01010a/b`): Rechannel ([Issue #2](https://github.com/SteveRodrigue/MCD/issues/2)), Commander targeting ([Issue #29](https://github.com/SteveRodrigue/MCD/issues/29)), Energy Channel, Cosmic Flight ([Issue #4](https://github.com/SteveRodrigue/MCD/issues/4)).
...
- [ ] **Remaining Core Player Card Issues:**
  ...
  - [x] **[Issue #24](https://github.com/SteveRodrigue/MCD/issues/24) / [Issue #119](https://github.com/SteveRodrigue/MCD/issues/119):** `feat(engine): Declarative Temporary Stat Modifier Auras — MODIFY_STAT, ActiveStatModifier, phase/round expiry, Vision 01068 PLAYER_CHOICE, ally stat bonus UI` ([ADR-0062](decisions/0062-declarative-temporary-stat-modifier-auras.md)) ✅ (Completed)
  - [x] **[Issue #23](https://github.com/SteveRodrigue/MCD/issues/23):** `feat(engine): Cross-player attachments & ownership (Combat Training 01057)` ([ADR-0066](decisions/0066-cross-player-attachments-and-card-ownership-invariants.md)) ✅ (Completed)

...
| `identityTraits` | `string[]` | Requires the identity (Hero or Alter-Ego side) to possess at least one matching trait. Evaluated dynamically via `hasPlayerTrait()`, taking into account printed traits and continuous upgrade trait additions (e.g. *Cosmic Flight* `01017` granting *Aerial*). | *"Play only if your identity has the Mystic trait."* | Doctor Strange, Scarlet Witch, Captain Marvel cards |

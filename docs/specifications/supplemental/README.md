...
| [**04. Combat & Threat**](./04_effects_combat_threat.md) | Damage & Scheme Control | `DEAL_DAMAGE`, `REMOVE_THREAT`, `PLACE_THREAT`, Overkill, Piercing, Guard, Crisis. |
| [**05. Zones & Cards**](./05_effects_zones_cards.md) | Hand, Deck & Discard Moves | `DRAW_CARDS`, `MODIFY_HAND_SIZE`, `SEARCH_AND_DRAW`, `DISCARD_TOP_DECK`, `PLAY_FROM_ZONE`. |
| [**06. Status & Economy**](./06_effects_status_economy.md) | Conditions & Resource Gen | `APPLY_STATUS`, `TOUGHNESS`, `RESOURCE_GENERATION`, `DOUBLE_RESOURCE`. |
...
| [**09. Sequences & Modals**](./09_sequences_and_prompts.md) | Chaining & Player Choices | `sequence: []` multi-action arrays, `PLAYER_CHOICE` Pop-Art decision prompt modals. |
